      ...
      <div class="parameter-section">
        <div class="parameter-row">
          ...
          <div class="parameter-column">
            ...
            <!-- 只读数据显示区域 -->
            <div v-if="showCementReadOnly" class="readonly-section">
              <!--<h6>水泥矿物组成参数</h6>-->
              <div class="input-group">
                <label>C3S含量:</label>
                <span>{{ cementReadOnlyValues.c3s }}</span>
                <label>%</label>
              </div>
              <div class="input-group">
                <label>C2S含量:</label>
                <span>{{ cementReadOnlyValues.c2s }}</span>
                                <label>%</label>

              </div>
              <div class="input-group">
                <label>C3A含量:</label>
                <span>{{ cementReadOnlyValues.c3a }}</span>
                                <label>%</label>

              </div>
              <div class="input-group">
                <label>C4A含量:</label>
                <span>{{ cementReadOnlyValues.c4af }}</span>
                                <label>%</label>
          ...
          <div class="parameter-column">
            ...
            <div v-if="showRegionOptions" class="sub-options">
             ...
             <!-- 只读数据显示区域 -->
            <div v-if="showRatioReadOnly" class="readonly-section">
              <div class="input-group">
                <label>水泥含量:</label>
                <span>{{ ratioReadOnlyValues.cement }}</span>
                                <label>kg/m³</label>

              </div>
              <div class="input-group">
                <label>粉煤灰量:</label>
                <span>{{ ratioReadOnlyValues.flyAsh }}</span>
                                                <label>kg/m³</label>

              </div>
              <div class="input-group">
                <label>矿粉含量:</label>
                <span>{{ ratioReadOnlyValues.slag }}</span>
                                                <label>kg/m³</label>

              </div>
              <div class="input-group">
                <label>水的含量:</label>
                <span>{{ ratioReadOnlyValues.water }}</span>
                                                <label>kg/m³</label>

              </div>
              <div class="input-group">
                <label>砂的含量:</label>
                <span>{{ ratioReadOnlyValues.sandRatio2 }}</span>
                                                <label>kg/m³</label>

              </div>
              <div class="input-group">
                <label>碎石含量:</label>
                <span>{{ ratioReadOnlyValues.sandRatio3 }}</span>
                                                <label>kg/m³</label>
...
const updateCharts = () => {
  ...
  type: 'value',
  min: 0,
  max: 672, // 将最大值从700小时调整为28天对应的小时数 (28天 * 24小时/天 = 672小时)
  interval: 96, // 设置刻度间隔为96小时（即4天）。这是实现固定刻度（0,4,8...28天）的关键
  axisLabel: {
    // 移除了在此处设置 interval 的代码
    formatter: function(value) {
      // 将小时值转换为天数。由于间隔已是整数天，使用 toFixed(0) 显示为整数
      return (value / 24).toFixed(0);
    }
  }
},
    yAxis: { 
            name: '放热量(10^4kJ/m3)',
            nameLocation: 'end',          // 名称置于轴线末端（右侧）
            nameGap: 30,                  // 名称与轴线的距离（默认20，增大可右移）
            nameTextStyle: {
                padding: [0, 0, 0, 40]     // 左内边距40px（向右偏移关键）
            }, 
            type: 'value',
            axisLabel: {
                formatter: function(value) {
                // 数值除以 10000，保留 1 位小数 + 添加单位
                return (value / 10000).toFixed(0) + ''; 
                },
                align: 'right',     // 标签右对齐
                padding: [0, 0, 0, 20], // 左内边距避免标签溢出
                // 移除无效的 offset 和 margin 配置（数值过大导致异常）
            }
            }
    ,
    series: [{
...
const handleFileUpload = (event) => {
  ...
  reader.onload = (e) => {
    try {
      ...
      // 获取第3行数据（行索引2，列索引1-4对应B-E列）
      const rowData = [
        worksheet['B3']?.v, // 第2列（B列）
        worksheet['C3']?.v, // 第3列（C列）
        worksheet['D3']?.v, // 第4列（D列）
        worksheet['E3']?.v  // 第5列（E列）
      ];
...
// 新增计算属性
const showCementReadOnly = computed(() => 
  selectedCement.value === 'import' || selectedCement.value === 'generate')

const showRatioReadOnly = computed(() => 
  selectedRatio.value === 'region')

// 水泥只读值
const cementReadOnlyValues = computed(() => {
  if (selectedCement.value === 'import') {
    return manualInput.value
  } else if (selectedCement.value === 'generate') {
    // 根据水泥类型返回预设值
    switch(selectedCementType.value) {
      case 'ordinary': return { c3s: 55, c2s: 20, c3a: 10, c4af: 8 }
      case 'slag': return { c3s: 60, c2s: 15, c3a: 12, c4af: 8 }
      case 'flyash': return {  c3s: 50, c2s: 30, c3a: 5, c4af: 12 }
      case 'composite': return { c3s: 45, c2s: 40, c3a: 4, c4af: 15 }
      default: return { c3s: 0, c2s: 0, c3a: 0, c4af: 0 }
    }
  }
...
// 配合比只读值
const ratioReadOnlyValues = computed(() => {
  if (selectedRatio.value === 'region') {
    const preset = ratioPresets[strengthGrade.value]?.[region.value]
    return preset || {
      cement: 0, flyAsh: 0, slag: 0, water: 0, 
      sandRatio2: 0, sandRatio3: 0
    }
  }
  return {
    cement: 0, flyAsh: 0, slag: 0, water: 0, 
...
{font-family: "SimSun", "Microsoft YaHei", "微软雅黑", serif; /* 设置微软雅黑 */ } 
/* 如果需要单独设置某个输入框的字体 */ 
.manual-input-section .input-group:nth-child(1) input 
{font-family: "SimSun", "Microsoft YaHei", "微软雅黑", serif; 
/* 设置微软雅黑 */ }

.manual-input-section .input-group label 
{ font-family: "SimSun", "Microsoft YaHei", "微软雅黑", serif; /* 设置微软雅黑 */ 
          font-size: 15px;
...
.import-section input[type="file"] {
  font-family: "SimSun", "Microsoft YaHei", "微软雅黑", serif; /* 确保文件选择框内的文本是微软雅黑 */
          font-size: 15px;

}

.import-section p {
  font-family: "SimSun", "Microsoft YaHei", "微软雅黑", serif; /* 确保提示文本是微软雅黑 */
          font-size: 15px;

}


.sub-options .input-group .custom-select {
width: 200px; /* 设置宽度为200px，可以根据需要调整 */
padding: 5px;
margin-left: 10px;
  font-family: "SimSun", serif; /* 设置字体为微软雅黑 */

}

.sub-options .input-group select {
padding: 1px;
margin-left: 5px;
width: 80px; /* 设置宽度为200px，可以根据需要调整 */
  font-family: "SimSun", serif; /* 设置字体为微软雅黑 */


}

.readonly-section .input-group span {
  font-family: "SimSun", serif; /* 设置字体为微软雅黑 */
        font-size: 15px;
        margin-left: -50px;
        text-align: center;
...
/* 新增只读区域样式-  background-color: rgba(247, 246, 245, 0.5); */
.readonly-section {
  padding: 0px;
  border-radius: 1px;
  border: 0px ;
...
h1 {
  ...
  font-size: 25px;
  white-space: nowrap;
    font-family: "SimSun", "Microsoft YaHei", "微软雅黑", sans-serif; /* 设置微软雅黑 */
